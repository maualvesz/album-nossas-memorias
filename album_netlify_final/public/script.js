...
async function loadPhotos() {
    try {
        ...
        alert('❌ Erro ao carregar fotos. Verifique sua conexão ou contate o suporte.');
        allPhotos = [];
    }
    renderPhotos();
}
...
async function savePhoto() {
    ...
    try {
        ...
        if (editIndex === -1) {
            ...
            console.log('Editando foto:', photoId);
...
async function deletePhoto(photoId) {
    if (confirm('Tem certeza que deseja excluir esta memória? 🌹')) {
        try {
            console.log('Deletando foto:', photoId);
...
function logout() {
    if (confirm('É você que quer sair? 🌹')) {
        ...
        localStorage.removeItem('album_photos');
